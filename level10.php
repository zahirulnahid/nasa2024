...
<head>
  ...
  <title>Orrery Level 10 - Pluto</title>
  <style>
...
<body>
  <div id="info-panel">
    <h1>Pluto</h1>
    <p>Click on Pluto to learn more about it!</p>
  </div>
  <button id="next-button" style="display: none;">Proceed to Level 11</button>
  ...
  <script>
    ...
    // Create other planets (Earth, Moon, Mars, Jupiter, Saturn, Uranus, Neptune)
    ...
    const plutoMaterial = new THREE.MeshStandardMaterial({ color: 0xcccccc }); // Light gray color for Pluto
    const pluto = new THREE.Mesh(plutoGeometry, plutoMaterial);
    
    const plutoOrbitRadius = 110; // Pluto orbit radius
    pluto.position.x = plutoOrbitRadius;
    scene.add(pluto);
    ...
    window.addEventListener('click', (event) => {
      ...
      const intersects = raycaster.intersectObjects([sun, earth, moon, mars, jupiter, saturn, uranus, neptune, pluto]);

      if (intersects.length > 0) {
        const clickedObject = intersects[0].object;

        // Display information based on clicked object
        if (clickedObject === pluto) {
          infoPanel.innerHTML = `
            <h1>Pluto</h1>
            <p><strong>Diameter:</strong> 2,377 kilometers</p>
            <p><strong>Orbit Time:</strong> 248 Earth years</p>
            <p><strong>Surface Temperature:</strong> -229°C</p>
            <p><strong>Fun Fact:</strong> Pluto has five known moons!</p>
          `;
    ...
    nextButton.addEventListener('click', () => {
      // Redirect to the next level (Level 11)
